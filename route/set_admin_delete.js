...
function set_admin_delete(req, res) {
    ...
    func.admin_check(db, req, res, function() {
        db.all("select set_data from user_data where user_name = ? and set_name = 'user_real_name' limit 1", [user_id], function(err, db_data) {
            if(db_data.length === 0) {
                ...
                db.run("update user_data set set_data = '' where user_name = ? and set_name = 'auth'", [user_id], function() {
                    res.json({
                        "req" : "ok",
                        "user_name" : user_id,
...
module.exports = {
    set_admin_delete : set_admin_delete
};