...
function func_study_preview() {
    document
        ...
        .addEventListener("click", function () {
            ...
            if (bbs_id !== "") {
                bbs_link +=
                    '<a class="text-decoration-none text-success" href="/board_read' +
                    xss_filter(bbs_id) +
                    '">(게시글)</a>';
            }
                    ...
                    <div class="col-md-9 shadow-sm rounded-5" style="margin: auto;">
                        <div class="container px-1">
                            ...
                            <div class="form-floating">
                                <input type="text" class="form-control" id="study_team_name" value="` +
        xss_filter(team_name) +
        `" placeholder="팀 이름">
                            ...
                            <br>    
                            <div class="form-floating">
                                <input type="datetime-local" class="form-control" id="study_date" value="` +
        xss_filter(date) +
        `" placeholder="날짜">
                            ...
                            <br>  
                            <div class="form-floating">
                                <input type="text" class="form-control" id="study_content" value="` +
        xss_filter(content) +
        `" placeholder="내용">
                            ...
                            <br>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="study_bbs_id" value="` +
        xss_filter(bbs_id) +
        `" placeholder="게시판 연계">
                                <label for="study_bbs_id">게시판 연계 (EX : /main/3)</label>
                            </div>
                            <br>
                            <button type="submit" class="btn btn-primary" id="study_save">저장</button>
                            <button type="submit" class="btn btn-outline-primary me-2" id="study_preview">미리보기</button>
                            <br>
                            <br>
...
if (document.location.pathname === "/study_add") {
    ...
    fetch("/api/study_read/" + url_encode(study_id))
        .then(function (res) {
            return res.json();
        })
        .then(function (text) {
