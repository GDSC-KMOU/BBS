...
function func_project_preview(){
    document.getElementById('project_preview').addEventListener("click", function() {
        ...
        document.getElementById('project_preview_field').innerHTML = `
            <div class="col">
                <div class="card shadow-sm">
                    <img src="` + xss_filter(file_url) + `">

                    <div class="card-body">
                        <p class="card-text">` + xss_filter(title) + `</p>
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <small class="text-muted">이름 (` + xss_filter(team_name) + `)</small>
                        </div>
                    </div>
                </div>
            </div>
...
function func_project_save(project_id = 0){
    document.getElementById('project_save').addEventListener("click", function() {
        let team_name = document.getElementById('project_team_name').value;
        let title = document.getElementById('project_title').value;
        let file_url = document.getElementById('project_file_url').value;
...
function func_project_editor(team_name = '', title = '', file_url = '', bbs_id = '') {
                    ...
                    <div class="col-md-9 shadow-sm rounded-5" style="margin: auto;">
                        <div class="container px-1">
                            ...
                            <div class="form-floating">
                                <input type="text" class="form-control" id="project_team_name" value="` + xss_filter(team_name) + `" placeholder="팀 이름">
                                <label for="project_team_name">팀 이름</label>
                            </div>
                            <br>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="project_title" value="` + xss_filter(title) + `" placeholder="제목">
                                <label for="project_title">제목</label>
                            </div>
                            <br>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="project_file_url"  value="` + xss_filter(file_url) + `" placeholder="파일 주소">
                                <label for="project_file_url">파일 주소</label>
                            </div>
                            <br>
                            <div class="form-floating">
                                <input type="text" class="form-control" id="project_bbs_id"  value="` + xss_filter(bbs_id) + `" placeholder="게시판 연계">
                                <label for="project_bbs_id">게시판 연계 (EX : /main/3)</label>
                            </div>
                            <br>
                            <button type="submit" class="btn btn-success" id="project_save">저장</button>
                            <button type="submit" class="btn btn-outline-success me-2" id="project_preview">미리보기</button>
                            <br>
                            <br>
...
if(document.location.pathname === '/project_add'){
    ...
    fetch("/api/project_read/" + url_encode(project_id)).then(function(res) {
        ...
        func_project_editor(text.team_name, text.title, text.file_url, bbs_id);
