...
if (document.location.pathname === '/project' || document.location.pathname.startsWith('/project/')) {
    ...
    let pageButton = '';
    fetch("/api/project_length").then((res) => {
        return res.json();
    }).then((data) => {
        let pageNum = Math.ceil(data.length / 12);
        for(let i =1; i <= pageNum; i++){
            pageButton += `<a class="text-decoration-none text-success p-1 m-1" href="/project/${String(i)}">${i}</a>`;
        }  
    })
    ...
    fetch("/api/project/" + url_encode(page)).then(function (res) {
            ...
            data += `
                <div class="col">
                    <div class="card card__project shadow-sm">
                        <img class="card-img-top" src="` + text[for_a]['file_url'] + `">
                        <div class="card-body">
        ...
        <section id="projectCarousel" class="container carousel slide carousel-fade mt-0" data-bs-ride="carousel">
            ...
            <div class="container rounded-5" style="background-color: #dbebe1">
            <div></div>
                <div class="container-lg">
                    <div class="project__border-top d-flex justify-content-end p-2 mb-3">
                    <span class="rounded px-1 border border-success">
                    <a href="/project_add" class="text-decoration-none text-success">
